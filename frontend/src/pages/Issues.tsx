...
const CommentsSection = ({ issueId, commentText, setCommentText, onCommentAdded }: CommentsSectionProps) => {
  ...
  // Проверка прав на добавление комментариев
  const canAddComment = () => {
    if (!user) return false
    const allowedRoles = ['ENGINEER', 'SITE_MANAGER', 'PROJECT_MANAGER', 'CHIEF_ENGINEER', 'DIRECTOR', 'CONTRACTOR']
...
const Issues = () => {
  ...
  // Доступно: Главный инженер, Руководитель проекта, Начальник участка, Прораб, Инженер, Мастер, Технадзор, Авторский надзор
  const canAddPhotoBefore = () => {
    if (!user) return false
    const allowedRoles = ['CHIEF_ENGINEER', 'PROJECT_MANAGER', 'SITE_MANAGER', 'FOREMAN', 'ENGINEER', 'MASTER', 'SUPERVISOR', 'OBSERVER']
    return user.is_superuser || allowedRoles.includes(user.role)
  }

  // Функция проверки прав на добавление фото "После" (отчет)
  // Доступно: Главный инженер, Руководитель проекта, Начальник участка, Прораб, Инженер, Мастер, Технадзор, Авторский надзор, Подрядчик
  const canAddPhotoAfter = () => {
    if (!user) return false
    const allowedRoles = ['CHIEF_ENGINEER', 'PROJECT_MANAGER', 'SITE_MANAGER', 'FOREMAN', 'ENGINEER', 'MASTER', 'SUPERVISOR', 'OBSERVER', 'CONTRACTOR']
    return user.is_superuser || allowedRoles.includes(user.role)
  }

  // Функция проверки прав на редактирование замечания
  // Доступно: Главный инженер, Руководитель проекта, Начальник участка, Прораб
  const canEditIssue = () => {
    if (!user) return false
    const allowedRoles = ['CHIEF_ENGINEER', 'PROJECT_MANAGER', 'SITE_MANAGER', 'FOREMAN']
    return user.is_superuser || allowedRoles.includes(user.role)
  }

  // Функция проверки прав на удаление замечания
  // Доступно: Главный инженер, Руководитель проекта, Начальник участка
  const canDeleteIssue = () => {
    if (!user) return false
    const allowedRoles = ['CHIEF_ENGINEER', 'PROJECT_MANAGER', 'SITE_MANAGER']
    return user.is_superuser || allowedRoles.includes(user.role)
  }

  // Функция проверки прав на принятие замечания (перевод в статус "Исполнено")
  // Доступно: Главный инженер, Руководитель проекта, Начальник участка, Прораб, Технадзор, Авторский надзор
  const canAcceptIssue = () => {
    if (!user) return false
    const allowedRoles = ['CHIEF_ENGINEER', 'PROJECT_MANAGER', 'SITE_MANAGER', 'FOREMAN', 'SUPERVISOR', 'OBSERVER']
    return user.is_superuser || allowedRoles.includes(user.role)
  }
