...
class UserViewSet(SoftDeleteViewSetMixin, viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['get'], url_path='contractors/export-template-v2',
            permission_classes=[permissions.IsAuthenticated])
    def export_contractors_template_v2(self, request):
        """
        Экспорт шаблона Excel v2 для импорта подрядчиков.

        Шаблон включает:
        - Лист "Инструкция" с подробным описанием процесса
        - Лист "Данные" с заголовками и dropdown для проектов
        - Поля: Email, Фамилия, Имя, Отчество, Компания подрядчика, Телефон, Объекты
        - Пример заполнения данных

        Permissions: CanManageContractorsExcel (от Прораба до Директора)
        """
        from .permissions import CanManageContractorsExcel
        from .utils.contractor_excel_handler import ContractorExcelHandler
        from datetime import datetime
        import logging
        import traceback
        ...
        try:
            user = request.user
            logger.info(f"[export_contractors_template_v2] User: {user.email}, Company: {user.company}")

            # Проверяем наличие компании ДО permission check
            if not user.company:
                logger.warning(f"[export_contractors_template_v2] User {user.email} has no company")
                return Response({
                    'error': 'У вас нет привязки к компании',
                    'details': 'Для работы с Excel необходимо привязать пользователя к компании через админ-панель'
                }, status=status.HTTP_400_BAD_REQUEST)
            ...
            # Проверяем права доступа
            logger.info(f"[export_contractors_template_v2] Checking permissions for user {user.email}")
            permission = CanManageContractorsExcel()
            if not permission.has_permission(request, self):
                logger.warning(f"[export_contractors_template_v2] Permission denied for user {user.email}")
                return Response({
                    'error': 'У вас нет прав для экспорта шаблона подрядчиков',
                    'details': 'Требуется роль от Прораба до Директора'
                }, status=status.HTTP_403_FORBIDDEN)

            # Создаем handler и генерируем шаблон
            logger.info(f"[export_contractors_template_v2] Creating ContractorExcelHandler")
            handler = ContractorExcelHandler(company=user.company)
            logger.info(f"[export_contractors_template_v2] Generating template")
            workbook = handler.generate_template_v2()

            # Создаем HTTP response с Excel файлом
            logger.info(f"[export_contractors_template_v2] Creating HTTP response")
            response = HttpResponse(
                content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
            )
            response['Content-Disposition'] = 'attachment; filename=contractors_import_template_v2.xlsx'

            # Сохраняем workbook в response
            logger.info(f"[export_contractors_template_v2] Saving workbook to response")
            workbook.save(response)

            logger.info(f"[export_contractors_template_v2] Success!")
            return response

        except Exception as e:
            logger.error(f"[export_contractors_template_v2] Exception: {str(e)}")
            logger.error(f"[export_contractors_template_v2] Traceback: {traceback.format_exc()}")
            return Response({
                'error': 'Ошибка при создании шаблона',
                'details': str(e)
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

    @action(detail=False, methods=['get'], url_path='contractors/export-v2',
            permission_classes=[permissions.IsAuthenticated])
    def export_contractors_v2(self, request):
        """
        ...
        try:
            ...
            if not permission.has_permission(request, self):
                return Response({
                    'error': 'У вас нет прав для экспорта подрядчиков',
                    'details': 'Требуется роль от Прораба до Директора'
                }, status=status.HTTP_403_FORBIDDEN)
            ...
            # Создаем HTTP response с Excel файлом
            current_date = datetime.now().strftime('%Y-%m-%d')
            filename = f'contractors_export_v2_{current_date}.xlsx'
            ...
            return Response({
                'error': 'Ошибка при экспорте подрядчиков',
                'details': str(e)
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

    @action(detail=False, methods=['get'], url_path='contractors/export-backup',
            permission_classes=[permissions.IsAuthenticated])
    def export_contractors_backup(self, request):
        """
        Создание backup всех подрядчиков компании.
        ...
        try:
            ...
            if not permission.has_permission(request, self):
                return Response({
                    'error': 'У вас нет прав для создания backup подрядчиков',
                    'details': 'Требуется роль от Прораба до Директора'
                }, status=status.HTTP_403_FORBIDDEN)
            ...
            # Создаем HTTP response с Excel файлом
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            filename = f'contractors_backup_{timestamp}.xlsx'
    ...
    @action(detail=False, methods=['post'], url_path='contractors/import-v2',
            permission_classes=[permissions.IsAuthenticated])
    def import_contractors_v2(self, request):
        """
        Импорт подрядчиков из Excel v2 с постоянными паролями.

        Поддерживает два режима:
        - mode=create: Массовое добавление новых подрядчиков (по умолчанию)
        - mode=update: Обновление данных существующих подрядчиков

        Процесс:
        1. Парсинг и валидация Excel файла
        2. Генерация постоянных паролей для новых подрядчиков
        3. Создание/обновление записей в БД (role='CONTRACTOR')
        4. Привязка к проектам
        5. Асинхронная отправка credentials на email
        ...
        try:
            ...
            if not permission.has_permission(request, self):
                return Response({
                    'error': 'У вас нет прав для импорта подрядчиков',
                    'details': 'Требуется роль от Прораба до Директора'
                }, status=status.HTTP_403_FORBIDDEN)
            ...
            for row_data in valid_rows:
                try:
                    ...
                    if mode == 'create':
                        ...
                        # Создаем подрядчика с role='CONTRACTOR'
                        new_contractor = User.objects.create(
                            email=email,
                            first_name=row_data['first_name'],
                            last_name=row_data['last_name'],
                            middle_name=row_data['middle_name'],
                            role='CONTRACTOR',  # Всегда CONTRACTOR
                            external_company_name=row_data['contractor_company'],  # Компания подрядчика
                            work_type=row_data['work_type'],  # Вид работ
                            phone=row_data['phone'],
                            company=user.company,  # Привязываем к компании пользователя
                            role_category='ITR_SUPPLY',  # Категория для подрядчиков
                            password=hashed_password,
                            is_active=True,
                            approved=True,
                        ...
                        if row_data['project_ids']:
                            ...
                            new_contractor.projects.set(projects)
                            projects_assigned += projects.count()

                        # Отправляем credentials на email асинхронно
                        send_permanent_credentials_email.delay(new_contractor.id, plain_password)
                        emails_sent += 1
                        ...
                        existing_contractor = User.objects.get(
                            email__iexact=email,
                            company=user.company,
                            role='CONTRACTOR'
                        )
                        ...
                        # Обновляем поля
                        existing_contractor.first_name = row_data['first_name']
                        existing_contractor.last_name = row_data['last_name']
                        existing_contractor.middle_name = row_data['middle_name']
                        existing_contractor.external_company_name = row_data['contractor_company']
                        existing_contractor.work_type = row_data['work_type']  # Обновляем вид работ
                        existing_contractor.phone = row_data['phone']

                        existing_contractor.save()
                        ...
                        if row_data['project_ids']:
                            ...
                            existing_contractor.projects.set(projects)
                            projects_assigned += projects.count()
            ...
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"[import_contractors_v2] Exception during import: {str(e)}")
            logger.error(f"[import_contractors_v2] Traceback: {traceback.format_exc()}")
